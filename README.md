...
**Super Asulin** replaced it with a fully custom platform engineered from a blank `style.css`: a custom homepage, a **sell-by-weight engine** for fresh produce, curated **one-click smart baskets**, a uniform **product spec-sheet system** with kosher & dietary badges, a coherent **liquid-glass design system**, abandoned-cart recovery in Hebrew, and structured data tuned for both search engines and AI answer engines — all **native right-to-left**.

Beyond the storefront, the project included full **catalog data operations**: deduplicating a corrupted import, cross-referencing a 7,000+-row product dataset, and building a **mobile barcode-scanning toolkit** so the owner can enrich products from the shop floor.
...
### 📷 Catalog Data Ops
Removed **575 duplicate products** from a bad import, cross-referenced a **7,334-row product dataset**, and shipped a **phone-camera barcode scanner** (HTML5, offline-capable) plus spreadsheet pipelines for shop-floor enrichment.
...
| 10 | 📷 **Barcode Toolkit** | Phone-camera scanner (HTML5) · pick-list & fast-scan CSV workflows · product-data matching |
...
Meta descriptions and focus keyphrases are generated by filter hooks at render time — not written into the database per product. Every future product inherits rich, localized meta automatically the moment it's published.
...
Product enrichment needed barcodes as a join key against a 7,334-row product dataset. Instead of dedicated hardware, the project shipped a self-contained HTML5 scanner — phone camera, live decode, local storage, CSV export — hosted over HTTPS so `getUserMedia` works anywhere in the store. Three workflows (pick-list, app, fast-scan) let the owner choose speed vs. certainty. A later cloud-synced version of the scanner is open source: [supermarket-barcode-scanner](https://github.com/galasulin/supermarket-barcode-scanner).
...
- 🎯 **Own visual identity** — a custom theme instead of a stock template.
- 🛒 **Sell-by-weight and one-tap baskets** — the two flows grocery shoppers use most, built natively.
...
Designed and built end-to-end by **[Gal Asulin](https://github.com/galasulin)** — architecture, UI/UX design system, front-end,<br/>WooCommerce integration, SEO layer, analytics, and catalog data tooling.

<br/>

<sub>Barcode tooling (open source): <a href="https://github.com/galasulin/supermarket-barcode-scanner">supermarket-barcode-scanner</a></sub>
